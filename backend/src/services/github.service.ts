...
async function getRepos(username: string, page: number = 1, perPage: number = 30): Promise<any[]> {
...
async function getRepoLanguages(username: string, repo: string): Promise<any> {
...
async function getEvents(username: string, page: number = 1): Promise<any[]> {
...
function computeRepoQuality(repo: any, languages: any, hasReadme: boolean): number {
...
function computeMaturity(user: any, repos: any[], recentActivity: number): number {
...
async function getFullAnalysis(username: string): Promise<any> {
  const [user, repos] = await Promise.all([
    getUser(username),
    getRepos(username),
  ])

  const repoAnalyses = await Promise.all(repos.slice(0, 20).map(async (repo: any) => {
  ...
  const recentActivity = events.filter((e: any) => {
  ...
  const topRepos = repoAnalyses.sort((a: any, b: any) => b.qualityScore - a.qualityScore)
  ...
  return {
    ...
    stats: {
      ...
      recentActivity,
      averageQuality: Math.round(repoAnalyses.reduce((s: number, r: any) => s + r.qualityScore, 0) / Math.max(1, repoAnalyses.length)),
    },
...
export { getUser, getRepos, getRepoLanguages, getReadme, getEvents, getFullAnalysis }
