...
interface StrengthResult {
  score: number
  factors: Record<string, ProfileFactor>
}
...
function computeProfileStrength(profile: any): StrengthResult {
  const factors: Record<string, ProfileFactor> = {}

  const skillsScore = (profile.skills && profile.skills.length > 0)
    ? Math.min(100, profile.skills.length * 8 + profile.skills.filter((s: any) => s.level > 50).length * 2)
    : 0
  ...
  const projectScore = (profile.projects && profile.projects.length > 0)
    ? Math.min(100, (profile.projects.length * 15) + profile.projects.reduce((s: number, p: any) => s + (p.impactScore || 0), 0) / Math.max(1, profile.projects.length))
    : 0
...
function generateInsights(profile: any, strengthResult: StrengthResult): Insight[] {
...
export { computeProfileStrength, generateInsights }
