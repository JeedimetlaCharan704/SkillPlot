...
interface SkillGapResult {
  ...
  gaps: Array<{ skill: string; weight: number; estimatedHours: number }>
  totalRequired: number
  matchedCount: number
}

function computeSkillGap(userSkills: any[], targetCareer: string): SkillGapResult | null {
  const path = CAREER_PATHS[targetCareer]
  if (!path) return null

  const userSkillNames = (userSkills || []).map((s: any) => (typeof s === 'string' ? s : s.name || '').toLowerCase()).filter(Boolean)
...
function computeAllCareerMatches(userSkills: any[]): SkillGapResult[] {
...
export { computeSkillGap, computeAllCareerMatches, CAREER_PATHS }
