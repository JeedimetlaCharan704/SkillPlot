...
function scoreSection (text, sectionName, keywordScore) {
...
function analyzeResume (textContent) {
  ...
  const weightMap = { critical: criticalWeight, important: importantWeight, 'nice-to-have': niceToHaveWeight }
...
async function analyzeResumeWithAI (textContent, targetRole) {
  if (!openaiService.isAvailable()) return null
  return openaiService.analyzeResume(textContent, targetRole)
}

module.exports = { analyzeResume, analyzeResumeWithAI, SKILLS_DB }
